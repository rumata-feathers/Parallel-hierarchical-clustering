...
static bool matrices_equal(const std::vector<std::tuple<int, int, double, int>>& a,
                            const std::vector<std::tuple<int, int, double, int>>& b,
                            double tol = 1e-9) {
    if (a.size() != b.size()) return false;
    for (size_t i = 0; i < a.size(); ++i) {
        if (std::get<0>(a[i]) != std::get<0>(b[i]) || std::get<1>(a[i]) != std::get<1>(b[i])) return false;
        if (std::abs(std::get<2>(a[i]) - std::get<2>(b[i])) > tol)         return false;
        if (std::get<3>(a[i]) != std::get<3>(b[i]))                         return false;
    }
    return true;
}
...
static std::vector<std::tuple<int, int, double, int>> expected_two_cluster() {
    return {
        {0, 1,  1.0, 2},
        {3, 4,  2.0, 2},
        {6, 2,  3.0, 3},
...
static void test_serial_expected_output() {
    ...
    if (matrices_equal(result, expected))
        ...
        for (auto& r : result)
            std::cerr << "    " << std::get<0>(r) << " " << std::get<1>(r) << " " << std::get<2>(r) << " " << std::get<3>(r) << "\n";
    }
}
...
static std::vector<std::tuple<int, int, double, int>> hac_ref(
    const std::vector<std::vector<double>>& dist,
    const std::vector<std::vector<double>>& data,
    Linkage linkage)
{
...
static void test_cuda_result_is_valid_linkage_matrix() {
    ...
    for (auto [lk, name] : std::vector<std::pair<Linkage, std::string>>{
        ...
        for (size_t i = 0; i < Z.size(); ++i) {
            CHECK(std::get<2>(Z[i]) >= 0.0, name + ": row " + std::to_string(i) + " distance < 0");
            if (i > 0)
                CHECK(std::get<2>(Z[i]) >= std::get<2>(Z[i - 1]) - 1e-9,
                      name + ": distances not non-decreasing at row " + std::to_string(i));
            CHECK(std::get<3>(Z[i]) >= 2.0, name + ": merged size < 2 at row " + std::to_string(i));
        }
    }
    if (n_failed == before) std::cout << "  PASS\n";
}
