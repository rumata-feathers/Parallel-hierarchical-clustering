    ...
    Linkage linkage, int n_threads)
{
        ...
        threads[t] = std::thread([&, t]()
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        if (cluster_nodes[k].empty() || k == merged_ci) continue;
                        double d = compute_cluster_dist(
    ...
    for (int step = 0; step < n - 1; ++step)
    {
        ...
        {
            double d = t_best[t];
            int ti = t_ci[t], tj = t_cj[t];
            if (ti == -1) continue;
            if (d < best_d || (d == best_d && ci != -1 &&
                (ti < ci || (ti == ci && tj < cj))))
            {
                best_d = d; ci = ti; cj = tj;
            }
        }
        // no clusters -> done
        if (ci == -1 || cj == -1)
        ...
        result.emplace_back(id[ci], id[cj], best_d,
            static_cast<int>(cluster_nodes[ci].size() + cluster_nodes[cj].size()));
        ...
        // mark removed cluster as INF so it is never picked again
        for (int k = 0; k < n; ++k)
            dist_matrix[cj][k] = dist_matrix[k][cj] =
    ...
    std::cerr << "[parallel] compute_ms=" << compute_ms
              << "  update_ms=" << update_ms
              << "  total_ms=" << (compute_ms + update_ms) << "\n";
